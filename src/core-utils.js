...
export const runApp = (
  ...
  skyUniforms = getDefaultUniforms(),
  composer = null
) => {
  ...
  window.addEventListener("resize", () => {
    ...
    // update uniforms.u_resolution
    if (skyUniforms.u_resolution !== undefined) {
      skyUniforms.u_resolution.value.x =
        window.innerWidth * window.devicePixelRatio;
      skyUniforms.u_resolution.value.y =
        window.innerHeight * window.devicePixelRatio;
    }
  });
  ...
  if ("ontouchstart" in window) {
    container.addEventListener("touchmove", mouseListener);
  } else {
  ...
  const animate = () => {
    ...
    app.updateScene(delta, elapsed);
    camera.position.x = Math.max(Math.min(1.2, camera.position.x + (( mouseX - camera.position.x ) * .00001)), -1);
    camera.position.y = Math.max(Math.min(1.2, camera.position.y + (( - mouseY - camera.position.y ) * .00001)), -1);
